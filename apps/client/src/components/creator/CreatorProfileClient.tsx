...
// Fallback used when API is unavailable or returns an error
...
export function CreatorProfileClient({ username }: CreatorProfileClientProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/creators/${username}`, {
      headers: {
      ...
      .then((data) => {
        setCreator(data ? { ...FALLBACK_CREATOR, ...data } : { ...FALLBACK_CREATOR, username });
        setIsLoading(false);
      })
      .catch(() => {
        setCreator({ ...FALLBACK_CREATOR, username });
        setIsLoading(false);
      });
  }, [username]);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-black font-sans pb-24">
        {/* Banner skeleton */}
        <div className="h-64 md:h-80 w-full bg-white/5 animate-pulse" />
    ...
    <div className="min-h-screen bg-black font-sans scrollbar-hide pb-24">
      ...
      <div className="container max-w-6xl mx-auto px-6 -mt-32 relative z-10">
        <header className="flex flex-col md:flex-row items-start md:items-end justify-between gap-8 pb-12">
            ...
            <div className="space-y-4 animate-fade-in">
               <div className="space-y-1">
                 <div className="flex items-center gap-2">
                    ...
                    {profile.isVerified && <CheckCircle2 className="w-6 h-6 text-[#9E398D]" />}
                 </div>
                 <p className="text-[#9E398D] font-black uppercase tracking-[0.2em] text-[10px]">@{profile.username}</p>
               </div>
               ...
               <div className="flex items-center gap-8 pt-2 overflow-x-auto whitespace-nowrap">
                  <div className="flex flex-col">
                     <span className="text-xl font-black text-white">{profile.followerCount.toLocaleString()}</span>
                     <span className="text-[10px] font-black uppercase text-neutral-600 tracking-widest">Followers</span>
                  </div>
                  <div className="flex flex-col">
                     <span className="text-xl font-black text-white">{profile.subscriberCount.toLocaleString()}</span>
                     <span className="text-[10px] font-black uppercase text-neutral-600 tracking-widest">Subscribers</span>
                  </div>
                  <div className="flex flex-col">
                     <span className="text-xl font-black text-white">{profile.contentCount}</span>
          ...
          <div className="flex flex-col items-center gap-4 w-full md:w-auto animate-fade-in">
             <button
        ...
        {!isFollowing && (
           <section className="bg-gradient-to-r from-[#9E398D]/10 to-[#521E49]/10 rounded-[2.5rem] p-12 border border-[#9E398D]/20 mb-12 flex flex-col md:flex-row items-center justify-between gap-12 animate-fade-in shadow-2xl">
              ...
              <div className="w-full md:w-auto text-center space-y-4">
                 <div className="text-5xl font-black text-white tracking-tighter">${profile.subscriptionPrice.toFixed(2)}<span className="text-sm font-medium text-neutral-600"> / month</span></div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {activeTab === 'POSTS' ? (
                ...
                <div className="col-span-full flex flex-col items-center justify-center py-20 text-center opacity-40">
                  ...
                  <p className="text-sm text-neutral-600 font-medium">{profile.displayName} hasn&apos;t finished any live sessions yet.</p>
                </div>
              )}
           </div>
        </section>
