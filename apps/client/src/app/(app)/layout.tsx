...
function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={clsx("flex h-screen bg-black overflow-hidden font-sans", isMobile ? "flex-col" : "flex-row")}>
      {/* Desktop Sidebar */}
      {!isMobile && (
        <aside className="w-64 border-r border-white/5 flex flex-col p-6 space-y-8 bg-[#0a0a0a]">
          ...
          <nav className="flex-1 space-y-2">
            ...
            <NavItem href="/search" icon={Search} label="Discover" isActive={pathname === '/search'} />
            <NavItem href="/subscriptions" icon={Heart} label="Subscribed" isActive={pathname === '/subscriptions'} />
            <NavItem href="/library" icon={Library} label="My Library" isActive={pathname === '/library'} />
            
            <div className="pt-8 pb-4">
              <span className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-neutral-600">Creator Studio</span>
            </div>
            
            <NavItem href="/creator/studio" icon={Video} label="Go Live" isActive={pathname === '/creator/studio'} />
            <NavItem href="/creator/upload" icon={PlusSquare} label="New Post" isActive={pathname === '/creator/upload'} />
          </nav>
      ...
      {isMobile && (
        <nav className="flex items-center justify-around p-4 bg-[#0a0a0a] border-t border-white/5 z-50">
          <Link href="/feed" className={clsx("p-2 rounded-xl transition-all", pathname === '/feed' ? "text-[#9E398D]" : "text-neutral-500")}>
            <Home className="w-6 h-6" />
          </Link>
          <Link href="/search" className={clsx("p-2 rounded-xl transition-all", pathname === '/search' ? "text-[#9E398D]" : "text-neutral-500")}>
            <Search className="w-6 h-6" />
          </Link>
          <div className="relative -mt-8">
             <Link href="/creator/studio" className="flex items-center justify-center w-14 h-14 rounded-full gradient-primary shadow-[0_10px_20px_-5px_rgba(158,57,141,0.5)] border-4 border-black">
